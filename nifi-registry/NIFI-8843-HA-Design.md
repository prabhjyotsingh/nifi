...
**Management port security:** `ManagementServerConfiguration` enforces loopback-only access (`127.0.0.1` / `::1`) at the security layer — requests from any other address receive `403 Forbidden` even if the management port is accidentally bound to a non-loopback interface.
...
| `ManagementServerConfiguration` | `actuator` | Management context security; loopback-only access control |
